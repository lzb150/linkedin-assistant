...
import { mkdirSync, rmSync, statSync, readFileSync, writeFileSync, utimesSync } from "node:fs";
...
// inside: a dead pid means a crashed run and is taken over immediately. An
// "alive" pid older than 2h is stale too — pids get reused, so an ancient
// lock whose pid happens to be running is far more likely a crash than a run.
// The holder touches the lock every 10 min, so "old mtime" means a dead holder,
// not a long run (LLM retries + a Mac sleeping mid-run have stretched runs past 2h).
export function acquireProfileLock(profileDir, { now = Date.now(), staleMs = STALE_LOCK_MS, heartbeatMs = LOCK_HEARTBEAT_MS } = {}) {
  ...
  try {
    // null (pid unknown, e.g. mkdir done but pid not yet written) is NOT dead.
    const stale = lockPidAlive(lock) === false || now - statSync(lock).mtimeMs > staleMs;
  ...
  // Two runs can both judge the lock stale, both rmSync, and both mkdir "win"
  // (the second rm deletes the first's dir). Re-reading our own pid narrows that
  // window to the gap between the two writes — if both land before both reads,
  // both runs still pass. A real CAS costs more code than the nanoseconds are worth.
  // ponytail: last-write-wins pid check, swap for O_EXCL on a pid file if two Chromiums ever do share a profile.
  ...
  if (!ownsLock()) {
    // A pid file we cannot read at all means nobody ever wrote one: our own
    // mkdir succeeded and the write did not, so this is our orphan. Left in
    // place it reads as "pid unknown, not dead" and blocks the profile for the
    // whole 2 h staleMs. A pid file naming someone else is their lock — leave it.
    let orphan = false;
    try { readFileSync(pidFile, "utf8"); } catch { orphan = true; }
    if (orphan) { try { rmSync(lock, { recursive: true, force: true }); } catch {} }
    throw new Error(`profile busy: another run holds ${lock}`);
  }
  const hb = setInterval(() => { try { utimesSync(lock, new Date(), new Date()); } catch {} }, heartbeatMs);
  hb.unref();
