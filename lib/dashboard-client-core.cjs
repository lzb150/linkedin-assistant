...
// "closed" is set by closed-check.mjs (board reports the vacancy inactive), never by a card button.
const STATUSES = ["viewed", "applied", "answered", "interview", "rejected", "closed"];
...
function entryToPatch(entry) {
  const e = entry || {};
  return { status: statusOfEntry(e), appliedAt: e.appliedAt || null, note: e.note || "" };
}
