...
// card is opened, "rejected" (✗) hides what is not yours, "closed" is set by
// closed-check.mjs (board reports the vacancy inactive), never by a card button.
const STATUSES = ["viewed", "rejected", "closed"];
...
function mergeEntryLocal(entry, patch) {
  ...
  const empty = !(STATUSES.includes(e.status) || (e.note && e.note.length));
  return empty ? null : e;
}
...
// clearing patch). A legacy cache saved before dirty-tracking has dirty ===
// null: fall back to the old one-time migration of entries the server lacks.
function offlinePatches(local, dirty, serverState) {
  const urls = dirty === null
    ? Object.keys(local).filter((u) => u !== "_meta" && !serverState[u])
    : dirty;
  return urls.map((url) => ({ url, patch: entryToPatch(local[url]) }));
}
