...
// copy identical (a missing field clears the server's value too). A status this
// build does not know (newer closed-check / dashboard) was never edited here, so
// it is left out of the patch rather than pushed as "new" (which would delete it).
function entryToPatch(entry) {
  const e = entry || {};
  const p = { note: e.note || "" };
  if (!e.status || STATUSES.includes(e.status)) p.status = statusOfEntry(e);
  return p;
}
