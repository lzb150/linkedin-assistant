...
const safeAltUrl = (u) => typeof u === "string" && u && !/[|,\s]/.test(u);
...
export function appendAltLink(file, source, url) {
  ...
  if (!fmBlock || !safeAltUrl(url) || fmBlock[0].includes(url)) return false;
...
export function buildApplication(job, scored, llm = null) {
  ...
  const salary = fmValue(extractSalary(job.text));
  ...
  const llmWhy = llm
    ? fmValue([String(llm.why || "").trim(), (llm.red_flags || []).length ? "⚠ " + llm.red_flags.join("; ") : ""]
        .filter(Boolean).join(" "))
    : "";
  const llmFront = llm ? `\nllm_score: ${fmValue(llm.score)}${llmWhy ? `\nllm_why: ${llmWhy}` : ""}` : "";
