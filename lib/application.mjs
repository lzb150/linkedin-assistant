...
export function appendAltLink(file, source, url) {
  ...
  if (!fmBlock || !safeAltUrl(url)) return false;
...
export function buildApplication(job, scored, llm = null) {
  ...
  const llmCover = typeof llm?.cover === "string" ? llm.cover.trim() : "";
  ...
  const llmFront = llm ? `\nllm_score: ${fmValue(llm.score)}${llmModel}${llmWhy ? `\nllm_why: ${llmWhy}` : ""}` : "";
...
`;
  ...
  const tail = createHash("sha1").update(String(job.url)).digest("hex").slice(0, 6);
