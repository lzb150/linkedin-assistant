...
export function buildApplication(job, scored, llm = null) {
  // Frontmatter is line-based; a title/company/location with a newline (e.g.
  // from a raw RSS title) would break parsing or inject fake keys. Collapse
  // whitespace at the write boundary regardless of which source it came from.
  const line = (s) => (s || "").replace(/\s+/g, " ").trim();
  job = { ...job, title: line(job.title), company: line(job.company), location: line(job.location) };
