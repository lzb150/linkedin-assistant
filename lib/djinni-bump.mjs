...
// nextCheckAt throttles /my/profile/ visits: one per day, hourly around the
// expected end of the cooldown (see nextBumpState).
...
// Garbage FIELDS are fine: dueForCheck/nextBumpState go through Date.parse.
// A corrupt FILE is not readable at all — treat it as "no state" (the next
// check simply happens sooner) but say so, since it is silently overwritten
// on the next successful bump.
export const readBumpState = (path, warn = console.warn) => {
  try { return readJson(path, null) || {}; }
  catch (e) { warn(`djinni-bump: ${path} unreadable (${e.message}) — treating as no bump state`); return {}; }
};
...
export function nextBumpState(prev, outcome, now = Date.now()) {
  ...
  // Anchor to the start of the current hour, not to `now`: launchd fires at :00
  // with minutes of jitter, and `now + period` only ever moves later, so the
  // next grid run (with less jitter) arrived before the deadline and was
  // skipped — daily checks drifted 18:09 → 19:02 → 20:00, hourly would be 2-hourly.
  const slot = Math.floor(now / HOUR_MS) * HOUR_MS;
  return { lastBumpAt, nextCheckAt: new Date(slot + wait).toISOString() };
}

// Logged-in probe shared by djinni-check.mjs and login.mjs. Djinni redirects
// protected pages to /login, and the /logout link appears only after a real
// login. (A `sessionid` cookie is set even for ANONYMOUS visitors, so cookie
// presence is NOT a reliable signal — it would "detect" a login before you
// type anything.)
export async function djinniLoggedIn(page) {
  try {
...
// Which unread threads deserve a banner, and its text. The seen set is the
// unread ids from the previous successful scan; a thread that is read (and
...
// `rawSeen` is whatever the seen file held — anything but an array means none.
export function freshThreads(threads, rawSeen) {
