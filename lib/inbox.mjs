...
// thread on load (url is a thread); `openListed` = that thread's id is still
// among the cards — when it is not, the filtered list has already dropped it as
// read, so it counts as one more and an empty list is then not drift.
...
//   scanOpenFirst — scan the already-open thread before the cards (it is unlisted,
//                   and opening it marked it read: this run is the last chance)
// SCAN_ALL walks the unfiltered list and leaves the badge alone.
// `openListed` has no default on purpose: the one caller always computes it,
// and a default of true would quietly report openDropped as false for any
// future caller that forgot it — the opposite of the safe answer.
export function unreadVerdict({ cards, autoOpened, openListed, emptyState, listFound, scanAll }) {
  if (scanAll) return { unreadCount: 0, counted: false, drift: false, scanOpenFirst: false };
  const openDropped = autoOpened && !openListed;
  return {
    unreadCount: cards + (openDropped ? 1 : 0),
    counted: listFound && (cards > 0 || emptyState || autoOpened),
    drift: listFound && cards === 0 && !emptyState && !autoOpened,
    scanOpenFirst: openDropped,
  };
}
