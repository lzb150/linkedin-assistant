...
import { normalizeProfile } from "./skills-profile.mjs";

const __dir = dirname(fileURLToPath(import.meta.url));
// Parsed once; application.mjs reads its `profile` block from here too.
// skills.json is hand-edited (the README says "edit freely"), so its types are
// not trusted: a quoted weight ("playwright": "5") would concatenate into the
// score as a string and pass every threshold, and a non-string role would throw
// mid-run. The same normalizer make-skills.mjs writes the file with reads it.
// ...and the file is read at import time by jobs.mjs, check.mjs and
// application.mjs alike, so an unguarded parse turned one stray comma into a
// raw SyntaxError that killed every entry point before a single job was
// gathered — and the message never named skills.json. Degrade the way the rest
// of the codebase does: say exactly what is wrong and how to regenerate it,
// then score with an empty profile (every job falls below threshold, so the run
// is visibly quiet rather than silently wrong).
export const profile = loadProfile(join(__dir, "..", "skills.json"));

function loadProfile(file, warn = console.error) {
  try {
    return normalizeProfile(JSON.parse(readFileSync(file, "utf8")));
  } catch (e) {
    warn(`⚠ ${file} unusable (${e.message}) — scoring with an EMPTY profile, so nothing will match. Regenerate it: node make-skills.mjs`);
...
export function looksLikeJobMessage(text) {
  ...
  const signals = [
    ...
    // The tool drafts replies in the sender's language (uk/ru/en), so the gate
    // in front of it cannot be English-only: a Ukrainian or Russian recruiter
    // DM matched nothing here, came back "not-job", and was stamped seen —
    // buried for the full TTL without ever being scored. Stems, not whole
    // words, to cover the case endings (вакансія/вакансію/вакансии).
    // No "робот"/"работ": "Вітаю з річницею роботи" is the very greeting this
    // gate exists to reject, and it carries the stem.
    "вакансі", "ваканси", "посад", "позиці", "позици",
    "шука", "ищем", "ищу ", "підбор", "подбор", "рекрутер", "рекрутинг",
    "співбесід", "собеседован", "інтерв", "интервью", "зарплат", "оклад",
    "ставка", "досвід", "опыт", "кандидат", "резюме", "пропозиц", "предложен",
    "віддалено", "удаленно", "наймаємо", "нанимаем",
  ];
