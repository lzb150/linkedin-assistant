...
//
//   buildReport({ now, days, packages, logText, health, llmMinScore })
//     → { text, notification }
...
// llmMinScore is the LLM gate from jobs.config.json: "strong" means a package
// that clears the gate as it stands now. A fixed 70 went stale the moment the
// gate reached 70 — every cold package cleared it, so the count said nothing.
// 0 (gate off, as when llm.minScore is unset) counts every scored package.
export function buildReport({ now = new Date(), days = 7, packages = [], logText = "", health = {}, runStats = [], llmMinScore = 0 }) {
  ...
  const strong = scored.filter((p) => Number(p.llm_score) >= llmMinScore).length;
  const byModel = {};
  ...
  const text = [
    ...
    `LLM scores: ${scored.length} scored${models}, ${strong} at ≥${llmMinScore} · top: ${top ? `${top.llm_score} ${top.title} @ ${top.company} (${top.source})` : "—"}`,
    `Source yield, median per run (last ${Math.max(0, ...Object.values(health).map((r) => (r || []).length))}): ${join(yields)}`,
  ].join("\n");

  const notification = `${run.written} packages (LLM ≥${llmMinScore}: ${strong}) · ${run.considered} new considered`;
