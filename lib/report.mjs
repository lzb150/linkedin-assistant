...
export function countRunStats(logText, { since = null, until = null } = {}) {
  ...
  for (const m of logText.matchAll(/^(\S+) {3}· skip \[\d+ \/ llm \d+\]/gm)) if (stampInWindow(m[1])) dropped++;
  for (const m of logText.matchAll(/^(\S+) {3}· llm failed for:/gm)) if (stampInWindow(m[1])) failed++;
