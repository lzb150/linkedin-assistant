...
//
//   buildReport({ now, days, packages, stateMap, logText, health })
//     → { text, notification }
...
export function buildReport({ now = new Date(), days = 7, packages = [], stateMap = {}, logText = "", health = {} }) {
  ...
  const scored = pk.filter((p) => Number.isFinite(Number(p.llm_score)) && p.llm_score !== undefined && p.llm_score !== "");
  ...
  const entries = Object.entries(stateMap).filter(([k]) => k !== "_meta").map(([, e]) => e || {});
  const applied = entries.filter((e) => inWindow(e.appliedAt, since)).length;
  const moved = (st) => entries.filter((e) => e.status === st && inWindow(e.updatedAt, since)).length;
  const appliedAllTime = entries.filter((e) => e.appliedAt).length;
  ...
  const text = [
    ...
    `LLM scores: ${scored.length} scored, ${strong} at ≥70 · top: ${top ? `${top.llm_score} ${top.title} @ ${top.company} (${top.source})` : "—"}`,
    `Pipeline this week: applied ${applied} · answered ${moved("answered")} · interview ${moved("interview")} · rejected ${moved("rejected")} (applied all-time: ${appliedAllTime})`,
    `Source yield, median per run (last ${Math.max(0, ...Object.values(health).map((r) => (r || []).length))}): ${join(yields)}`,
  ].join("\n");

  const notification = `${pk.length} packages (LLM ≥70: ${strong}) · applied ${applied}, answered ${moved("answered")} · ${run.considered} new considered`;
