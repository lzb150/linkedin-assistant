...
export function countRunStats(logText) {
  ...
  for (const m of logText.matchAll(/^\S+ Done\. Considered (\d+) new, wrote/gm)) { runs++; considered += Number(m[1]); }
  const dropped = (logText.match(/^\S+ {3}· skip \[\d+ \/ llm \d+\]/gm) || []).length;
  const failed = (logText.match(/^\S+ {3}· llm failed for:/gm) || []).length;
