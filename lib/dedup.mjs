...
export function normalizeCompany(s) {
  ...
  // A name made only of legal-form tokens ("Group", "Co") is still a name:
  // keep it rather than collapsing it into the blank-company bucket.
...
export function canonicalKey(job) {
  const tokens = normalizeTitle(job.title)
    .split(" ")
    .filter((t) => t && !/^\d{3,}$/.test(t)) // req IDs are 3+ digits; "Engineer 2" keeps its level
    .flatMap((t) => TITLE_ALIASES[t] || [t]);
  return `${normalizeCompany(job.company)}::${[...new Set(tokens)].sort().join(" ")}`;
}
...
export function dedupeJobs(jobs) {
  ...
  for (const job of jobs) {
    // Blank company: title alone would merge unrelated postings from unrelated
    // employers, so scope the key to the posting's own host+path instead.
    const key = normalizeCompany(job.company) ? canonicalKey(job) : `${urlKey(job.url)}::${canonicalKey(job)}`;
