// Cross-source job de-duplication. Dependency-free, no network. Not pure:
// dedupeJobs mutates the matched records in place (altLinks, text) rather than
// cloning, so the caller's own job objects come back enriched — deliberate, and
// pinned by the idempotence test.
// The same vacancy posted on DOU/Djinni/LinkedIn arrives with a
...
// Case/punctuation/whitespace only. A parenthetical qualifier like
// "(Playwright)" survives as the token "playwright", so distinct roles at the
// same company do not merge. Also the first step of normalizeCompany.
...
// used by the seen set and the dashboard collapse.
// Company: first token only — boards spell the same employer differently
// ("EPAM" / "EPAM Systems", "NIX" / "NIX Solutions", "DIGIS" / "DIGIS, a Fiverr
// company"): 92 same-title pairs in the seen store differed only that way, each
// a duplicate package. ponytail: two employers sharing a first word AND a
// title across boards would merge into one card with an alt-link; accepted.
export function canonicalKey(job) {
  ...
  return `${company ? company.split(" ")[0] : urlKey(job.url)}::${[...new Set(tokens)].sort().join(" ")}`;
}
