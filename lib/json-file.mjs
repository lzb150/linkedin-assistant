...
export function writeTextAtomic(path, text) {
  ...
  try {
    ...
    // The mode lands on every file this function writes, existing ones
    // included: rename(2) replaces the target with this tmp inode, so a target
    // that was 0644 is 0600 after its next rewrite. Intended — the contents are
    // the same either way — but it is a change to files already on disk, not
    // a creation-only rule like the mkdirSync modes in the scripts.
