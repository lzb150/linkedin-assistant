// Atomic JSON write: sibling temp file + fsync + rename, so a reader (or a
// crash mid-write / power loss) never sees a half-written or empty file.
...
export function writeJsonAtomic(path, value, indent = 0) {
  ...
  try {
    ...
    try {
      writeSync(fd, JSON.stringify(value, null, indent));
