...
// A value that IS a number but sits outside the knob's range is clamped, which
// is right — but doing it silently meant the file said one thing and the run did
// another (llm.minScore 500 → 100, concurrency 0 → 1) with nothing in the log.
// Both cases are now reported; a value that needs no correction stays quiet.
export function knob(name, v, fallback, range = [-Infinity, Infinity], log = console.log) {
  const out = numberIn(v, fallback, range);
  if (v == null) return out;
  if (Number.isNaN(numberIn(v, NaN, range))) {
    log(`⚠ jobs.config.json: ${name} is ${JSON.stringify(v)}, not a number — using ${fallback}`);
  } else if (out !== Number(v)) {
    log(`⚠ jobs.config.json: ${name} is ${JSON.stringify(v)}, outside [${range[0]}, ${range[1]}] — using ${out}`);
  }
  return out;
}
