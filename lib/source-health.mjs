...
// { source: [counts] } from disk; garbage in → empty history out.
...
export function detectDegradations(history, summary, { skipped = [] } = {}) {
  const out = [];
  for (const [source, b] of Object.entries(summary.sources)) {
  ...
  // A source that vanishes from the run entirely — disabled by a config typo,
  // or a renamed key — never reaches summary.sources, so both rules above are
  // blind to it: the loop simply stops seeing it and the source disappears for
  // good, silently. Its history is the evidence that it used to work.
  // `skipped` names the sources this run left out on purpose (DOU_ONLY, a
  // profile busy with check.mjs): absent by design, not by a config typo —
  // without it every hourly DOU-only run raised "linkedin: not run at all".
  for (const [source, hist] of Object.entries(history)) {
    if (source in summary.sources || skipped.includes(source)) continue;
    if (median(hist) >= MIN_MEDIAN) out.push({ source, found: 0, median: median(hist), reason: "absent", runs: hist.length });
  }
  return out;
}
...
export function formatAlert(degradations) {
  return "⚠️ " + degradations
    .map((d) => {
      if (d.reason === "never-succeeded") return `${d.source}: never returned results in ${d.runs} runs`;
      if (d.reason === "absent") return `${d.source}: not run at all (recent median ${d.median}) — disabled or misspelt in jobs.config.json?`;
      return `${d.source}: ${d.found} found (recent median ${d.median})`;
    })
    .join("; ");
}
