...
async function initState() {
  try {
    ...
    // the next reconnect. localStorage is only cleared after every patch went
    // through, so a mid-loop failure leaves the not-yet-pushed urls in place.
    state = loadLocal();
...
async function patchEntry(url, patch) {
  ...
  if (online) {
    ...
    try { state = await postState({ url, patch }); saveLocal(); return; }   // refresh the offline mirror
    catch { markOffline(); }
  }
...
async function advanceLastVisit() {
  ...
  if (online) { try { state = await postState({ _meta: { lastVisit: nowIso } }); saveLocal(); } catch {} }
