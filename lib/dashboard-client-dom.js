...
const STATUS_KEY = 'jobStatus';        // offline store + read mirror of the server: { url: {status,note} | "viewed" }
...
// Auto-status never overrides a ✗ you set, nor reopens a board-closed card.
async function autoStatus(card, status){ const st = statusOf(card.dataset.url); if (st === 'rejected' || st === 'closed') return; await setStatus(card, status); }
...
// on open reads as broken; ✗ and Closed stay hidden until both tabs are deselected.
...
function restoreFilters(){
  ...
  // Only statuses with a header button (dashboard.mjs): a saved filter for a
  // removed one ("applied", "closed") would show an empty board.
...
function applyFilter(){
  const counts = { all: 0, new: 0, viewed: 0, rejected: 0, closed: 0 };   // ✗/closed counted, no header button (they only show under All)
...
function currentTheme(){
  const pinned = document.documentElement && document.documentElement.dataset && document.documentElement.dataset.theme;
