...
function restoreFilters(){
  ...
  // Statuses that no longer exist (a saved "applied" filter) would show an empty board.
  statusSel.clear(); (f.status || []).filter((v)=>['new', ...STATUSES].includes(v)).forEach((v)=>statusSel.add(v));
...
function applyFilter(){
  const counts = { all: 0, new: 0, viewed: 0, rejected: 0, closed: 0 };
