...
async function initState() {
  ...
  try {
    ...
    for (const body of offlinePatches(local, savedDirty, state)) {
    ...
    // store; `dirty` still holds every url not yet pushed. A cache that predates
    // dirty-tracking has no list — seed it with every entry so the one-time
    // "push what the server lacks" migration still happens on reconnect.
    state = local;
    if (savedDirty === null) for (const u of Object.keys(state)) if (u !== '_meta') dirty.add(u);
    saveLocal();
    markOffline();
  }
}
...
async function setStatus(card, status){
  ...
  renderCard(card); applyFilter(); renderFunnel(); showNudges();
}
// Auto-status never downgrades any post-applied card.
// Auto-status never downgrades a post-applied card, nor reopens a board-closed one.
async function autoStatus(card, status){ const st = statusOf(card.dataset.url); if (POST_APPLIED.includes(st) || st === 'closed') return; await setStatus(card, status); }
...
// ---- "Did you apply?" nudge ---------------------------------------------
// People apply on the job site and never come back to press Applied (3 months:
// 696 viewed, 0 applied). Opening a job or copying its letter arms a per-card
// prompt that stays until the card is marked applied or dismissed. Armed urls
// live in localStorage because the page is regenerated every run.
const NUDGE_KEY = 'applyNudge';   // { url: armedAtMs }
const NUDGE_TTL = 3 * 86400000;   // after 3 days it is noise, not a reminder
function nudges() { try { const n = JSON.parse(localStorage.getItem(NUDGE_KEY) || '{}'); return n && typeof n === 'object' ? n : {}; } catch { return {}; } }
function saveNudges(n) { try { localStorage.setItem(NUDGE_KEY, JSON.stringify(n)); } catch {} }
function armNudge(card) {
  const url = card && card.dataset.url;
  const st = url && statusOf(url);
  if (!url || POST_APPLIED.includes(st) || st === 'closed') return;   // nothing to apply to on a board-closed vacancy
  const n = nudges(); n[url] = Date.now(); saveNudges(n); showNudges();
}
function disarmNudge(url) { const n = nudges(); delete n[url]; saveNudges(n); showNudges(); }
// Re-sync every card's prompt with the store; drops arms that are applied or stale.
function showNudges() {
  const n = nudges(); let changed = false;
  for (const card of cards) {
    const url = card.dataset.url, el = card.querySelector('.apply-nudge');
    if (!url || !el) continue;
    let show = url in n;
    if (show && (POST_APPLIED.includes(statusOf(url)) || Date.now() - n[url] > NUDGE_TTL)) { delete n[url]; changed = true; show = false; }
    el.hidden = !show;
  }
  if (changed) saveNudges(n);
}
async function nudgeYes(card) { await setStatus(card, 'applied'); disarmNudge(card.dataset.url); }
function nudgeNo(card) { disarmNudge(card.dataset.url); }
...
function renderFunnel() {
  ...
  if (el) el.textContent = formatFunnel(computeFunnel(rows));
}
...
function applyFilter(){
  const counts = { all: 0, new: 0, viewed: 0, applied: 0, answered: 0, interview: 0, rejected: 0, closed: 0 };
  ...
  for (const k of ['all', 'new', ...STATUSES]) { const el = document.getElementById('cnt-'+k); if (el) el.textContent = counts[k]; }
  saveFilters();
}
