...
export function buildJobPrompt(resume, job, lang) {
  ...
  const stripTag = (s) => { const re = /<\s*\/?\s*vacancy\b[^>]*>/gi; let prev; do { prev = s; s = s.replace(re, ""); } while (s !== prev); return s; };
  const safe = (v, n) => stripTag(String(v ?? "")).replace(/\s+/g, " ").trim().slice(0, n);
  const text = stripTag(String(job.text ?? "")).slice(0, 6000);
