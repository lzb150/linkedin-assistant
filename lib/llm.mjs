...
function llmCallOnce(prompt, { model = "haiku", exec = execFile, log = () => {} } = {}) {
  return new Promise((resolve) => {
    try {
      ...
      exec(
        ...
        // so 60s timed out 51 of 53 calls in one week. The CLI ignores SIGTERM,
        // which then stalled the whole run for up to an hour — hence SIGKILL.
        { timeout: 180_000, killSignal: "SIGKILL", maxBuffer: 1024 * 1024, cwd: tmpdir() },
        (err, stdout, stderr) => {
          if (err) {
            const why = err.killed ? `timeout ${180}s, ${err.signal}` : err.code === undefined ? err.message : typeof err.code === "number" ? `exit ${err.code}` : err.message;
    ...
    } catch (e) { log(`llm failed: ${e.message}`); resolve(null); }
  });
}
