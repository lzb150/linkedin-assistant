...
export function extractDivByClass(html, className) {
  ...
  const m = new RegExp(`<div\\b[^>]{0,2048}class="[^"]{0,512}${className}[^"]{0,512}"[^>]{0,2048}>`, "i").exec(html);
  if (!m) return "";
  let depth = 1;
...
export async function fetchText(url, log, label, doFetch = globalThis.fetch) {
  const res = await doFetch(url, { headers: HEADERS, signal: AbortSignal.timeout(FETCH_TIMEOUT) });
...
export async function bodyText(res) {
  if (Number(res.headers?.get?.("content-length")) > MAX_BODY) throw new Error(`body over ${MAX_BODY} bytes`);
  const text = await res.text();
  if (text.length > MAX_BODY) throw new Error(`body over ${MAX_BODY} bytes`);
  return text;
}
