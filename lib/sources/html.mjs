...
export function stripHtml(s) {
  ...
  // never leaks into scored text. Only tag-like tokens (<letter, </, <!) — a
  // prose "salary < 5000 … > 3 years" must not lose the text in between.
  return decodeEntities((s || "").replace(/<[a-zA-Z!\/][^<>]{0,4096}>/g, " ").replace(/<[a-zA-Z!\/][^<>]*>/g, " "))
    .replace(/\s+/g, " ")
    .trim();
}
...
export function extractDiv(html, openRe) {
  ...
  if (!m) return "";
  let depth = 1;
  // Two bounds so a div with a huge attribute list (inline JSON state) still
  // counts toward depth instead of desynchronising the nesting.
  const tag = /<\/?div\b(?:[^>]{0,2048}|[^<>]{0,65536})>/gi;
...
// Resolve a scraped href against `base` and return it only if it stays on the
// site (base's host minus "www.", or a subdomain, http(s) only, no userinfo or
// explicit port); null otherwise.
// new URL() keeps an absolute href verbatim, so this is what stops a hostile
// listing link from being stored as a job — or, for the browser sources,
// page.goto()'d in the shared context that holds the LinkedIn session.
export function siteUrl(href, base) {
