  ...
  for (const keywords of config.searches || []) {
    ...
    try {
      ...
      for (const card of cards.slice(0, max)) {
        const read = async (sel) => { try { const el = await card.$(sel); return el ? (await el.innerText()).trim() : ""; } catch { return ""; } };
        const title = await read(SEL.title);
        const href = await card.$(SEL.title).then((el) => el?.getAttribute("href")).catch(() => null);
        ...
        out.push({
          ...
          title,
          company,
          url: new URL(href, "https://www.glassdoor.com").href.split("?")[0],
          location,
