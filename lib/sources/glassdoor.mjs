...
/**
 * Resolve a scraped card href to an absolute glassdoor.com listing URL, or null.
 * new URL() ignores the base for absolute hrefs, so a link that resolves off
 * glassdoor.com (or to a non-http scheme) is dropped rather than stored as a
 * "glassdoor" job — same guard as the robota.ua and work.ua fetchers.
 * Keeps ?jl=<jobId>: the bare /job-listing/… path is 403 on Glassdoor.
 */
export function jobUrl(href) {
  if (typeof href !== "string" || !href) return null;
  let u;
  try { u = new URL(href, "https://www.glassdoor.com"); } catch { return null; }
  if (!/^https?:$/.test(u.protocol) || !/(^|\.)glassdoor\.com$/.test(u.hostname)) return null;
  return u.href.replace(/(\?jl=\d+).*$/, "$1");
}
