...
export async function fetchDjinni(config, log = console.log, { skip = () => false } = {}) {
  ...
  for (const search of config.searches || []) {
    try {
      let n = 0, read = 0;   // `read` counts pages that actually yielded jobs, not the one we broke on
      for (let p = 1; p <= pages; p++) {
        ...
        out.push(...jobs); n += jobs.length; read++;
      }
      log(`  Djinni search ok (${n} over ${read} page${read === 1 ? "" : "s"}): ${search}`);
    } catch (e) {
