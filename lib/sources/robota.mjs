...
/**
 * Resolve a scraped card href to an absolute robota.ua vacancy URL, or null.
 * new URL() ignores the base for absolute hrefs, and the result is later
 * page.goto()'d in the shared context that holds the LinkedIn session — so a
 * link that resolves off robota.ua (or to a non-http scheme) is dropped, the
 * way the work.ua fetcher pins its origin.
 */
export function jobUrl(href) {
  if (typeof href !== "string" || !href) return null;
  let u;
  try { u = new URL(href.split("?")[0], "https://robota.ua"); } catch { return null; }
  if (!/^https?:$/.test(u.protocol) || !/(^|\.)robota\.ua$/.test(u.hostname)) return null;
  return u.href;
}
...
export async function fetchRobota(page, config, log = console.log) {
  ...
  for (const search of config.searches || []) {
    ...
    try {
      ...
      // `break` (not return) so whatever earlier searches collected still gets
      // de-duplicated and enriched below.
      ...
      // Cloudflare/Angular occasionally stalls the first load; one reload fixes it.
      await page.waitForSelector(SEL.card, { timeout: 15000 }).catch(async () => {
        log("    cards not rendered, reloading once...");
  ...
  // Skipped when Cloudflare blocked us: every detail page would just time out.
  if (!blocked && config.fullDescription !== false && jobs.length) {
