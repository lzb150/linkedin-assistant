  ...
  for (const search of config.searches || []) {
    ...
    try {
      ...
      for (const c of cards) {
        ...
        // new URL() ignores the base for absolute hrefs, and job.url is later
        // page.goto()'d in the shared context that holds the LinkedIn session —
        // never follow a scraped link off robota.ua (work.ua pins its origin too).
        const u = new URL(c.href.split("?")[0], "https://robota.ua");
        if (!/(^|\.)robota\.ua$/.test(u.hostname)) continue;
        n++;
        out.push({
          source: "robota",
          title: c.title,
