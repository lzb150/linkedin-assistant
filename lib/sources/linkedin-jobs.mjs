...
import { composeText, uniqueByUrl, oneLine } from "./html.mjs";
import { knob } from "../filters.mjs";
...
// Runs inside the page (serialised — keep it self-contained): pull everything
// we need from one card in one round trip.
export function readCard(card, SEL) {
  const first = (sel) => card.querySelector(sel);
  const text = (el) => ((el && el.innerText) || "").trim().split("\n")[0];
  // The title link holds the title twice (visible + a visually-hidden copy) and
  // innerText joins them on one line: "Senior SDETSenior SDET". Halve it.
  const unDouble = (t) => (t.length % 2 === 0 && t.length && t.slice(0, t.length / 2) === t.slice(t.length / 2) ? t.slice(0, t.length / 2) : t);
  const tEl = first(SEL.title);
  let location = "";
  for (const lEl of card.querySelectorAll(SEL.location)) { location = text(lEl); if (location) break; }
  return { title: unDouble(text(tEl)), href: (tEl && tEl.getAttribute("href")) || "", company: text(first(SEL.company)), location };
}

// A card's title link is normally a relative path, but a sponsored or
// third-party card can carry an absolute href pointing anywhere. Only LinkedIn
// itself is accepted: anything else returns "" and the card is skipped, rather
// than being stored as job.url and surfaced in the dashboard and the package.
export function jobUrlFrom(href) {
  const path = String(href || "").split("?")[0];
  if (!path.startsWith("http")) return path ? `https://www.linkedin.com${path}` : "";
  try {
    const h = new URL(path).hostname;
    return h === "linkedin.com" || h.endsWith(".linkedin.com") ? path : "";
  } catch { return ""; }
}

// `skip(job)` → true for a vacancy the caller already knows (seen store,
// excluded title): the card is returned from its list fields alone — no click,
// no 1.8 s pause, no description — so the caller can still re-stamp it as seen.
export async function fetchLinkedInJobs(page, config, log = console.log, { sleep = defaultSleep, searchTimeoutMs = SEARCH_TIMEOUT, skip = () => false } = {}) {
  const out = [];
  const max = knob("linkedin.maxResults", config.maxResults, 8, [1, Infinity], log);
  ...
  for (const search of config.searches || []) {
    ...
    const scan = async () => {
      ...
      // Wait for the first card instead of a fixed 2.5 s (a fake page without
      // waitForSelector, or a page with no cards, just falls through).
      try { await page.waitForSelector(SEL.card, { timeout: 10000 }); } catch {}
      ...
      // never clicked.
      const urls = new Set();
      let taken = 0, skipped = 0, stagnant = 0, rounds = 0, cardCount = 0;
      while (taken < max && stagnant < 2 && !expired()) {
        ...
        for (const card of cards) {
          ...
          // readCard runs inside the page, where oneLine does not exist, so the
          // scraped text is scrubbed here. dou.mjs (oneLine) and djinni.mjs
          // (stripHtml's \s+ collapse) already do this; LinkedIn was the one
          // source without it, and its titles are the most attacker-shaped of
          // the three — anyone can post a listing. readCard's .split("\n")[0]
          // drops a literal LF but keeps \r and U+2028/U+2029, which JS treats
          // as line terminators: jobs.mjs logs the title raw and report.mjs
          // counts that log with ^-anchored /gm patterns, so a crafted title
          // forges lines in the weekly digest.
          for (const k of ["title", "company", "location"]) fields[k] = oneLine(fields[k]);
          // No href → jobUrl would collapse to bare "https://www.linkedin.com".
          if (!fields.title || !fields.href) continue;
          const jobUrl = jobUrlFrom(fields.href);
          if (!jobUrl) continue;
          if (urls.has(jobUrl)) continue;
          urls.add(jobUrl);
          added++; taken++;

          // Click to load the description panel — unless the caller already
          // knows this vacancy. Re-check the deadline here: the evaluate above
          // may have crossed it, and a stale loop must not click a card on the
          // next search's page.
          if (expired()) break;
          let desc = "";
          if (skip({ title: fields.title, company: fields.company, url: jobUrl })) skipped++;
          else {
            try {
              await card.click({ timeout: ACTION_TIMEOUT });
              // Correlate the panel with THIS card. A blind 1.8 s wait had
              // nothing tying it to the click, so a slow re-render handed the
              // PREVIOUS job's description to scoring and to the LLM cover
              // letter. LinkedIn puts the clicked job's id in the url.
              const jobId = jobUrl.match(/\/jobs\/view\/(\d+)/)?.[1];
              if (jobId) {
                await page.waitForURL((u) => u.href.includes(jobId), { timeout: ACTION_TIMEOUT }).catch(() => {});
                await sleep(400);   // the url flips before the panel finishes painting
              } else {
                await sleep(1800);  // no id to wait on: the old blind delay
              }
          ...
          out.push({
            ...
            text: composeText({ title: fields.title, company: fields.company, location }, desc),
          });
        }
      ...
      log(`    found ${cardCount} cards, ${taken} unique after ${rounds} scroll(s) (capped at ${max})${skipped ? `, ${skipped} already known — not opened` : ""}`);
    };
    // The deadline can win this race, and the abandoned scan() keeps going:
    // when the next search destroys the context its page.goto / page.$$ reject
    // with nobody awaiting them, and that unhandled rejection kills the very
    // run the deadline exists to salvage. Claim the late failure here; a
    // rejection that arrives before the deadline is still reported by the catch
    // below, and expired() keeps the two from logging the same error twice.
    const running = scan();
    running.catch((e) => { if (expired()) log(`    LinkedIn search: late error after the deadline — ${e?.message || e}`); });
    try {
