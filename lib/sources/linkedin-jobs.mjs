...
export async function fetchLinkedInJobs(page, config, log = console.log, { sleep = defaultSleep, searchTimeoutMs = SEARCH_TIMEOUT } = {}) {
  ...
  for (const search of config.searches || []) {
    ...
    const expired = () => Date.now() - t0 > searchTimeoutMs;
    let timer;
    const deadline = new Promise((_, reject) => {
      timer = setTimeout(() => reject(new Error(`search deadline ${Math.round(searchTimeoutMs / 1000)}s exceeded — keeping what was gathered`)), searchTimeoutMs);
    });
    const scan = async () => {
