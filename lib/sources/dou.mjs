...
import { decodeEntities, stripHtml, fetchText, uniqueByUrl, oneLine } from "./html.mjs";
...
export async function fetchDou(config, log = console.log) {
  ...
  for (const feed of config?.feeds || []) { // jobs.mjs passes config.dou, which may be absent
    try {
      const xml = await fetchText(feed, log, "DOU feed");
      if (!xml) continue;
