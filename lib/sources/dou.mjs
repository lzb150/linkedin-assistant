...
export function parseRss(xml) {
  ...
  while ((m = re.exec(xml))) {
    ...
    if (vIdx !== -1) {
      ...
      // With 3+ parts a middle one may be the salary ("$3000-5000") — drop those
      // from the company; a 2-part rest keeps its comma-bearing company as-is.
      const companyParts = restParts.length > 1 ? restParts.slice(0, -1) : restParts;
      company = (restParts.length > 2 ? companyParts.filter((p) => !/[$€₴]|\d{3,}/.test(p)) : companyParts).join(", ");
