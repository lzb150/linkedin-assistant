...
const TEMPLATES = {
  en: {
    ...
    maybe: (name) =>
      ...
      `(tech stack, scope of work, engagement type and rate)? I want to make sure it's a good fit before we proceed.\n\nBest regards,\n${NAME}`,
  },
  ru: {
    relevant: (name, skills) =>
    ...
    maybe: (name) =>
      ...
      `(стек, объём работ, формат сотрудничества и ставка)? Хочу убедиться, что это хороший мэтч, прежде чем двигаться дальше.\n\nС уважением,\n${NAME}`,
  },
  uk: {
    relevant: (name, skills) =>
    ...
    maybe: (name) =>
      ...
      `(стек, обсяг робіт, формат співпраці та ставка)? Хочу переконатися, що це гарний збіг, перш ніж рухатися далі.\n\nЗ повагою,\n${NAME}`,
  },
};
...
export function buildDraft(thread, scored) {
  ...
  // thread.name may be missing (LinkedIn markup drift); every frontmatter value
  // is collapsed to one line so a newline in a name cannot inject a key.
  const name = firstName(fmValue(thread.name));
  const threadName = fmValue(thread.name) || "Unknown";
  const replyBody =
    scored.verdict === "relevant" ? tpl.relevant(name, skills) : tpl.maybe(name);

  const md = `---
thread: ${threadName}
url: ${fmValue(thread.url) || "(open from LinkedIn messaging)"}
generated: ${when}
reply_language: ${lang}
...
penalties: ${fmValue(scored.penalties.join(", ")) || "—"}
attach_resume: ${attach ? "YES — " + fmValue(RESUME_PATH) : "no (ask for details first)"}
---

# ${threadName} — ${scored.verdict.toUpperCase()} (score ${scored.score})
...
`;
  ...
  const safeName = threadName.replace(/[^a-z0-9]+/gi, "_").slice(0, 40);
  // safeName is empty for Cyrillic names, and two threads drafted in the same
  // minute would collide — a short hash of url/name keeps filenames unique.
  const tail = createHash("sha1").update(String(thread.url || thread.name || "")).digest("hex").slice(0, 6);
  const stamp = when.slice(0, 16).replace(/[:T]/g, "");
  return { filename: `${stamp}_${scored.verdict}_${safeName}_${tail}.md`, markdown: md };
}
