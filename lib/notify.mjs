...
// check.mjs, djinni-check.mjs, jobs.mjs and closed-check.mjs). macOS banners are
...
// string or AppleScript source — they carry scraped job-board text. (macOS:
// osascript reads them from `argv` inside the script, because JSON.stringify is
// not an AppleScript escaper: \b, \f and \u00XX made it a syntax error and the
// banner vanished. terminal-notifier shows no banners on macOS 26; Linux:
// notify-send, with `--` so a message starting with "-" is not an option.)
function fallback(title, message, platform, exec) {
  const [cmd, args] = platform === "linux"
    ? ["notify-send", ["--", title, message]]
    : ["osascript", ["-e", "on run argv", "-e", "display notification (item 1 of argv) with title (item 2 of argv)", "-e", "end run", "--", message, title]];   // "--": a message starting with "-" is not an option
...
export function notify(title, message, {
  ...
  try {
    ...
    writeTextAtomic(file, JSON.stringify({ title, message })); // atomic: the daemon never reads a partial file
    ensure();
  } catch (e) {
