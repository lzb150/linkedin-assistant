...
// After a currency-led floor the upper bound may carry its own symbol; after a
// bare floor it may not be bare itself, and the caller must demand a currency
// word behind it. Shared by both range forms so they cannot drift apart.
const UPPER_AFTER_CUR = `(?:${C}${N}|${NK}|${N3})`;
const UPPER_NEEDS_CW = `(?:${NK}|${N3})`;

// Group 1: range — $3,000–$5,000 / $3k–5k / 3000–5000 USD / €3 000 – €5 000
const RANGE = new RegExp(
  `(?:${C}${N}\\s*[-–—]\\s*${UPPER_AFTER_CUR}(?:\\s+${CW})?|${N}\\s*[-–—]\\s*${UPPER_NEEDS_CW}\\s+${CW})${RS}`,
  "i"
);
...
const UA_RANGE = new RegExp(
  `${NOT_WORD_LEFT}від\\s+(?:${C}${N}(?:\\s+${CW})?\\s*до\\s+${UPPER_AFTER_CUR}(?:\\s+${CW})?|${N}(?:\\s+${CW})?\\s*до\\s+${UPPER_NEEDS_CW}\\s+${CW})${RS}`,
  "iu"
);
