...
- **Work.ua** — через публічну дошку вакансій (простий fetch, без логіна, без браузера)
...
- **Work.ua** — публічна дошка вакансій (`work.ua`), рендериться на сервері й читається простим fetch, за формою як Djinni. Кожен пошук — повний URL сторінки пошуку (напр. `https://www.work.ua/jobs-qa+automation/`). `workua.enabled=false` вимикає джерело.
