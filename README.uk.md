...
- **Work.ua** — через спільну сесію браузера (Cloudflare, лише повні прогони, без логіна)
- **Robota.ua** — через спільну браузерну сесію (за Cloudflare, потребує `HEADFUL=1`)
- **Glassdoor** — через спільну браузерну сесію (за Cloudflare, лише повні прогони; Україна, пошук за ключовими словами)
...
- **Work.ua** — публічна дошка вакансій (`work.ua`), за Cloudflare, тож читається через спільний Playwright-браузер під час повних прогонів (як Robota.ua; без логіна). Кожен пошук — повний URL сторінки пошуку (напр. `https://www.work.ua/jobs-qa+automation/`). `workua.enabled=false` вимикає джерело.
- **Robota.ua** — за Cloudflare, який жорстко блокує headless Chrome, тож джерело дає результати лише на запусках з `HEADFUL=1` (інакше пропускається з підказкою в лозі). Читається через той самий Playwright-браузер, що й LinkedIn, логін не потрібен. Кожен пошук — повний URL пошуку (напр. `https://robota.ua/zapros/qa-automation/ukraine`). `robota.enabled=false` вимикає джерело.
- **Glassdoor** — за Cloudflare, тож читається через спільний Playwright-браузер під час повних прогонів (як Robota.ua; без логіна). Кожен пошук — рядок ключових слів (локація — Україна); клік по картці підвантажує повний опис. `glassdoor.enabled=false` вимикає джерело.
