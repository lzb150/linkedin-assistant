...
- «Холодні» відгуки проходять попередній поріг за ключовими словами — `minScore` (типово 18) + `requireRole` — і далі поріг LLM (`llm.minScore`, 70), який і робить справжній відбір. `excludeLocation` і далі лишає віддалену вакансію DOU, де просто перелічено іноземний офіс. Djinni оцінюється за власним сегментом країн: кандидат має мати право жити в Україні («Україна», «Країни Європи та Україна», «Весь світ»), тож «Тільки віддалено · Канада, Польща, Сербія» чи «Країни ЄС» відкидаються навіть за «віддалено».
