...
яка не ваша; **Closed** ставить перевірка закритих вакансій нижче. Це вся
модель — конвеєра applied/answered/interview немає (кілька справжніх відгуків
...
живі лічильники за статусами і фільтр за ними.
...
власний фільтр і не відкриваються назад кліком. Картки ✗ не чіпаються.
...
закрилися 14+ днів тому, до `applications/archive/` — цю теку ніхто не читає,
тож дашборд позбувається тих карток. Щоденний запуск `closed-check.mjs` робить
таке ж архівування сам (`CLOSED_ARCHIVE_DAYS` для налаштування).
