...
| Нагадування про фолоу-ап (`jobs-followup`)            | щодня 09:30            |
| Тижневий звіт (`jobs-report`)                         | понеділок 09:00        |
| Демон бейджа в Dock (`jobs-badge`)                    | постійно (KeepAlive)   |

Запуск `jobs.mjs`, який записав нові пакети, завершується macOS-сповіщенням з
підсумком (запуск без нового мовчить — про те, що планувальник живий, скажуть
час оновлення дашборда і тижневий звіт), плюс
...
**Автобамп:** той самий запуск підтримує профіль свіжим — Djinni дозволяє «Bump My Profile» (підняти профіль у результатах пошуку рекрутерів) періодично (кожні 7 днів, як спостережено; кнопка сама каже, коли можна). Раз на день скрипт перевіряє кнопку на `djinni.co/my/profile/` і натискає її, щойно вона активна — стан кнопки є джерелом істини, тож будь-яка частота, яку дозволяє Djinni, підхоплюється автоматично. Підтверджує модальне вікно й показує банер «Profile bumped». Стан тротлінгу лежить у `djinni-bump-state.json` (у .gitignore); збій бампу ніколи не впливає на сканування непрочитаного.
...
`localStorage` і показує бейдж **"offline — not saved to disk"**. Перед першим
записом кожного дня стан копіюється в `job-state.YYYY-MM-DD.bak` (зберігаються
останні 7) — щоб відкотити невдалий день, скопіюйте знімок поверх
`job-state.json`. Сервер живе
між оновленнями; запускач із Dock (`open-dashboard.sh`) порівнює його час
старту (`/health`) із датами файлів сервера й перезапускає його, якщо код
новіший, тож ручний `pkill` після оновлення не потрібен.
...
показується як "applied 5d ago". Оскільки сам відгук відбувається на сайті
вакансії, легко так і не повернутися натиснути Applied: клік по **Open job**
або **Copy letter** вмикає на картці підказку **"Did you apply to this job?"**,
яка переживає перегенерацію дашборда (зберігається в localStorage браузера),
доки ви не відповісте **Yes, mark Applied** чи **Not yet**, або картка не
перейде у стадію після Applied іншим шляхом; через 3 дні вона зникає сама.
Рядок у шапці підсумовує воронку для
...
навичками. Чіпи джерел (LinkedIn / DOU / Djinni / Jooble / Robota / Work.ua / Glassdoor) і пресети
