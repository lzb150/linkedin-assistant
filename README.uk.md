...
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/banner-dark.uk.svg">
  <img alt="linkedin-assistant — локальний радар вакансій: шукає, оцінює й готує чернетки, але нічого не надсилає" src="docs/banner-light.uk.svg" width="960">
</picture>

<br>

[![Джерела](https://img.shields.io/badge/%D0%B4%D0%B6%D0%B5%D1%80%D0%B5%D0%BB%D0%B0-DOU_%C2%B7_Djinni_%C2%B7_LinkedIn-1f6feb?style=flat-square&labelColor=161b22)](#пошук-вакансій--jobsmjs)
[![Дані](https://img.shields.io/badge/%D0%B4%D0%B0%D0%BD%D1%96-%D0%BB%D0%B8%D1%88%D0%B5_%D0%BB%D0%BE%D0%BA%D0%B0%D0%BB%D1%8C%D0%BD%D0%BE-238636?style=flat-square&labelColor=161b22)](#ключові-принципи)
[![Автовідправка](https://img.shields.io/badge/%D0%B0%D0%B2%D1%82%D0%BE%D0%B2%D1%96%D0%B4%D0%BF%D1%80%D0%B0%D0%B2%D0%BA%D0%B0-%D0%BD%D1%96%D0%BA%D0%BE%D0%BB%D0%B8-9e6a03?style=flat-square&labelColor=161b22)](#ключові-принципи)
[![macOS](https://img.shields.io/badge/macOS-launchd-8957e5?style=flat-square&labelColor=161b22)](#розклад-launchd)
[![Node.js](https://img.shields.io/badge/Node.js-20%2B-30363d?style=flat-square&labelColor=161b22)](#одноразове-налаштування)
[![Ліцензія](https://img.shields.io/badge/%D0%BB%D1%96%D1%86%D0%B5%D0%BD%D0%B7%D1%96%D1%8F-MIT-0969da?style=flat-square&labelColor=161b22)](LICENSE)

[Що вміє](#що-вміє) · [Налаштування](#одноразове-налаштування) · [Пошук](#пошук-вакансій--jobsmjs) · [Дашборд](#дашборд--dashboardmjs) · [Розклад](#розклад-launchd) · [Інша професія](#адаптація-під-іншу-професію)
...
- **HTML-дашборд** — усі вакансії на одній сторінці, відсортовані за
  ...
  нотатки, вкладки New/Viewed, фільтр за джерелом, пошук, підсвітка нових і
  кнопка копіювання листа
...
| Сервер стану дашборда (`state-server`)               | постійно (KeepAlive)   |
...
cp ~/шлях/до/резюме.txt resume.txt   # звичайний текст; на ньому тримаються оцінка й листи
node make-skills.mjs             # збирає skills.json — ЩО шукати
$EDITOR jobs.config.json         # ДЕ шукати: фіди, пошуки, ваша країна

node login.mjs                   # ВИ логінитесь вручну (з 2FA). Пароль ніде не зберігається.
```

**`resume.txt`** — єдиний файл, який застосунок не вигадає за вас. Вставте повний
текст: `make-skills.mjs` читає його, щоб скласти профіль ключових слів, крізь який
першим проходить кожна вакансія, а локальна LLM потім переоцінює за ним кожну
знахідку. Хочете спершу подивитись — `node make-skills.mjs --print`; у будь-якому
разі прочитайте результат один раз і підправте ваги.

**`jobs.config.json`** у поставці налаштований на вакансії з автоматизації
тестування. Це той крок, який ніхто за вас не зробить: це вибір дощок і формулювань
запитів, а не факт, який можна витягти з резюме. Змініть фіди DOU, пошукові URL
Djinni та запити LinkedIn на свою сферу й вкажіть `candidateCountry` — див.
[Адаптація під іншу професію](#адаптація-під-іншу-професію).

`login.mjs` відкриває справжній браузер. Залогіньтесь повністю (з 2FA); він сам
помітить вхід, збереже сесію у `.browser-profile/` і закриється. Без входу протягом
6 хвилин він завершується, нічого не зберігши — просто запустіть ще раз. Щоб увійти
ще й на Djinni: `node login.mjs djinni`.

Далі перевірте, що все працює, перш ніж ставити на розклад:

```bash
node jobs.mjs        # один прохід по всіх увімкнених джерелах
node dashboard.mjs   # зібрати applications/index.html і подивитись, що знайшлось
```
...
`notify-state.json`. **Застосунок Jobs** (`Jobs.app`) постійно
...
![Чип Djinni і пошуковий запит звужують список](docs/filters.png)

**Тема** — слідує системній світлій/темній; кнопка в шапці закріплює одну з них
...
Відсутня збірка Chromium (пакет Playwright оновили, а браузер ні, або хтось
стер `~/Library/Caches/ms-playwright`) лікується сама: перший браузерний
скрипт, що впав з «Executable doesn't exist», пише в лог `installing
chromium…`, скачує збірку (~100 МБ, до 5 хвилин) і стартує повторно.
`npx playwright install chromium` руками потрібен лише коли той запуск
завершився банером «Playwright browser build missing … (install failed: …)» —
не було мережі або Mac заснув під час завантаження.
...
| `open-dashboard.sh`   | Помічник кліку в Dock: перегенерувати дашборд, відкрити браузер. |
