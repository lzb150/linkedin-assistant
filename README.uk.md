...
- **Jooble** — через офіційний Jooble API (безкоштовний ключ, структурований JSON); **типово вимкнено** — сніпети короткі й оцінюються погано, увімкніть `jooble.enabled`, щоб спробувати
- **Work.ua / Robota.ua / Glassdoor** — через спільну браузерну сесію, але Cloudflare блокує їх у headless-режимі, тож **типово вимкнені** (вмикаються з `HEADFUL=1`, з'являтиметься вікно Chrome)
...
## Ключові принципи
- 🔒 **Безпека:** пароль ніде не зберігається (логінитесь один раз самі), все локально; єдиний ключ — безкоштовний API-ключ Jooble, і лише якщо це джерело увімкнене
...
DOU_ONLY=1 node jobs.mjs   # пропустити скрейпінг LinkedIn (DOU + Djinni, а також Jooble, якщо увімкнено, працюють — повністю чисто щодо ToS)
HEADFUL=1 node jobs.mjs    # видиме вікно Chrome (також потрібне для Work.ua / Robota.ua / Glassdoor)
```

Заплановані прогони типово headless — без вікна браузера. Джерела, які Cloudflare блокує в headless Chrome (Work.ua, Robota.ua, Glassdoor), постачаються з `enabled: false`; щоб їх увімкнути, поставте `enabled: true` у `jobs.config.json` і `export HEADFUL=1` у `run-jobs.sh`, змирившись із вікном Chrome під час кожного повного прогону.
...
- **Jooble** — офіційний Jooble API (`jooble.org/api`). Jooble за Cloudflare, тож API — єдиний підтримуваний шлях. Потрібен **безкоштовний** API-ключ з [jooble.org/api/about](https://jooble.org/api/about), задається через змінну середовища `JOOBLE_API_KEY` (у `run-jobs.sh`, який у gitignore — ніколи не комітьте ключ). Ключі прив'язані до ринку — конфіг фіксує `apiHost: "ua.jooble.org"` (український ринок, кожна вакансія доступна з України), тож ключ реєструйте саме там. Пошуки — пари `{ keywords, location }` у `jobs.config.json`: `''` = вся Україна, `"віддалено"` = лише віддалені, або місто. `jooble.enabled=false` вимикає джерело.
- **Work.ua** — *типово вимкнено (у headless повертає порожні сторінки результатів).* Публічна дошка вакансій (`work.ua`), за Cloudflare, читається через спільний Playwright-браузер під час повних прогонів з `HEADFUL=1` (без логіна). Кожен пошук — повний URL сторінки пошуку (напр. `https://www.work.ua/jobs-qa+automation/`). `workua.enabled=true` вмикає джерело.
- **Robota.ua** — *типово вимкнено (Cloudflare жорстко блокує headless Chrome).* Дає результати лише на запусках з `HEADFUL=1` (інакше пропускається з підказкою в лозі). Читається через той самий Playwright-браузер, що й LinkedIn, логін не потрібен. Кожен пошук — повний URL пошуку (напр. `https://robota.ua/zapros/qa-automation/ukraine`). `robota.enabled=true` вмикає джерело.
- **Glassdoor** — *типово вимкнено (у headless сторінка результатів не завантажується до таймауту).* За Cloudflare, читається через спільний Playwright-браузер під час повних прогонів з `HEADFUL=1` (без логіна). Кожен пошук — рядок ключових слів (локація — Україна); клік по картці підвантажує повний опис. `glassdoor.enabled=true` вмикає джерело.
...
- **Фільтр закордонних вакансій** — дошки публікують і вакансії, фізично
  розташовані за кордоном (DOU позначає їх «за кордоном»; на Jooble UA
  трапляється «Краків, Польща»). Вакансія, чия локація містить будь-який
...
- «Холодні» відгуки проходять попередній поріг за ключовими словами — `minScore` (типово 18) + `requireRole` — і далі поріг LLM (`llm.minScore`, 70), який і робить справжній відбір. Вакансію з позначкою «віддалено» `excludeLocation` ніколи не відкидає за іноземний офіс у списку.
...
приглушеною позначкою «· closed») і не відкриваються назад кліком. Картки ✗ не чіпаються.
Адреси Jooble/Work.ua/Robota.ua/Glassdoor
