...
- **LLM-переоцінка та персональні супровідні листи** — найсильніші keyword-збіги
  ...
  (`enabled`, `model`, `minKeywordScore`, `maxPerRun`). Картки з LLM-оцінкою
...
→ Interview**, або **Rejected** на будь-якому кроці. Позначка Applied записує
дату і показує її як "applied 5d ago". Рядок у шапці підсумовує воронку для
